...
FILENAME = f"figure2-supp-allfits-hcp.pdf"
MODEL = "Colorless"
DATASET = 'hcp1200'
...
def metric_of_interest(model, metric, valid=None):
    ...
    return np.asarray(model.get_metrics()[metric])[valid]

# Compute upper and lower bounds for each metric's plot.  We want them to all
# be the same for a given metric.
maxes = {met: max([max(metric_of_interest(session, met)) for m in cmodels for session in m[1]]) for met in metrics}
mins = {met: min([min(metric_of_interest(session, met)) for m in cmodels for session in m[1]]) for met in metrics}
bounds = {}
for met in metrics:
