...
data_unrelated = cdatasets.HCP1200Unrelated()
...
ts_metrics_vals = {"meanar1": np.mean(data_unrelated.get_ar1s(), axis=1),
                   "loglmbda": np.log(data_unrelated.get_lmbda()),
                   "lmbda": data_unrelated.get_lmbda(),
                   "floor": data_unrelated.get_floor(),
                   "meancor": data_unrelated.get_cmstats()['meancor'],
                   "varcor": data_unrelated.get_cmstats()['varcor'],
                   "kurtcor": data_unrelated.get_cmstats()['kurtcor'],
}
ts_metrics = ["lmbda", "floor", "spatialmetrics", "meanar1", "allmetrics"] # List them because dicts are unordered

graph_metric_values = data_unrelated.get_metrics()
graph_metric_values.update(data_unrelated.get_cmstats())
...
c.add_text("Regional TA-$\Delta_1$ by\nregional TA-$\Delta_1$ reliability", Point(.5, 1, "axis_reliability_vs_ar1")+Vector(0,.3, "cm"), ha="center", **titlestyle)
...
ax.invert_yaxis()
c.add_text("Correlation with\nregional TA-$\Delta_1$", Point(.5, 1, "axis_ar1_corr_boxplots")+Vector(0,.3, "cm"), ha="center", **titlestyle)
