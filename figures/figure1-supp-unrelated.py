...
FILENAME = "figure1-unrelated.pdf"

import cdatasets
data = cdatasets.HCP1200Unrelated()
data_rep = cdatasets.HCP1200Unrelated(3)
datatrt = cdatasets.HCP1200UnrelatedKindaLikeTRT()
...
if not os.path.exists(fn):
    reliabilities = {k : icc_full(datatrt.base.get_subject_info()[datatrt.inds]['subject'], metrics_vals[k]) for k in all_metrics}
...
if os.path.exists(fn):
    ...
    subjects = np.asarray(datatrt.base.get_subject_info()[datatrt.inds]['subject'])
    run = datatrt.base.get_subject_info()[datatrt.inds]['run']
    scan = datatrt.base.get_subject_info()[datatrt.inds]['scan']
...
if not os.path.exists(fn): # No need to regenerate if it exists
    ...
    region_reliabilities['ar1'] = [icc_full(datatrt.base.get_subject_info()[datatrt.inds]['subject'], np.asarray(datatrt.get_ar1s())[:,i])[0] for i in range(0, datatrt.N_regions())]
    for met in ['mean', 'var', 'kurt']:
        region_reliabilities[met] = [icc_full(datatrt.base.get_subject_info()[datatrt.inds]['subject'], datatrt.get_nodal_cmstats()[met][:,i])[0] for i in range(0, datatrt.N_regions())]
    for met in ['degree', 'centrality']:
        region_reliabilities[met] = [icc_full(datatrt.base.get_subject_info()[datatrt.inds]['subject'], datatrt.get_nodal_metrics()[met][:,i])[0] for i in range(0, datatrt.N_regions())]
